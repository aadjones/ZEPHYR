...
VECTOR3_FIELD_3D BlockCompressVectorField(const VECTOR3_FIELD_3D& V, COMPRESSION_DATA& compression_data) { 
  ...
  for (int component = 0; component < 3; component++) {            
    ...
    cout << "Compression component " << component << endl;
...
FIELD_3D DoBlockCompression(FIELD_3D& F, COMPRESSION_DATA& compression_data) {
  ...
  // use this dummy to pass the dims into AssimilateBlocks later on

  vector<FIELD_3D> blocks = GetBlocks(F);     
  cout << "Doing block DCT.." << endl;
  DoBlockDCT(blocks);
  cout << "Done with block DCT. " << endl;

  int blockNumber = 0;
  cout << "Running through each block..." << endl;
  ...
  cout << "Done with the block loop!" << endl;
